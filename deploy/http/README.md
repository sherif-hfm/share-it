...
The `Deploy Share-It to VM` GitHub Actions workflow runs on the `shareit-vm` self-hosted runner. Only pushes to `main` trigger it; pull requests do not. Each run checks out that commit on the VM, builds the Docker image, recreates the app if its image changed, and checks the HTTP health endpoint. The runner needs Docker access and outbound access to GitHub; no inbound SSH or registry is needed.

...
public class BrowserTests(BrowserFixture fixture)
{
    ...
    public async Task Mount_drive_guide_has_platform_commands_copy_feedback_and_keyboard_access()
    {
        ...
        await dialog.GetByText("Connection details for other clients", new() { Exact = true }).ClickAsync();
        ...
        await dialog.GetByText("Connection details for other clients", new() { Exact = true }).ClickAsync();
        await dialog.GetByText("View command", new() { Exact = true }).ClickAsync();
        foreach (var platform in new[] { "Windows", "Linux", "macOS" })
        {
            ...
            var command = await dialog.Locator(".mount-command:not(.mount-full-command)").InnerTextAsync();
            var extension = platform == "Windows" ? "ps1" : "sh";
            Assert.Contains($"{fixture.Url}/{session.Code}/mount.{extension}", command);
            Assert.StartsWith(platform == "Windows" ? "irm " : "(set -o pipefail; curl -fsSL ", command);
            Assert.EndsWith(platform == "Windows" ? " -ErrorAction Stop | iex" : " | bash)", command);
            ...
            await dialog.GetByText("Advanced: full command", new() { Exact = true }).ClickAsync();
            var full = await dialog.Locator(".mount-full-command").InnerTextAsync();
            Assert.Contains(expectedUrl, full); Assert.Contains("--read-only", full);
            Assert.Contains("--webdav-user " + session.Code, full);
            Assert.Contains("--dir-cache-time 15s", full); Assert.Contains("--poll-interval 0", full);
            if (platform == "Windows") Assert.Contains("$shareItDrive = 'S:'", full);
            else Assert.Contains("mkdir -p", full);
            await dialog.GetByRole(AriaRole.Button, new() { Name = "Copy full command", Exact = true }).ClickAsync();

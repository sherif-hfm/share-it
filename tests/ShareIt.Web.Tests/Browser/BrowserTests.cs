...
public class BrowserTests(BrowserFixture fixture)
{
    ...
    public async Task Mount_drive_guide_has_platform_commands_copy_feedback_and_keyboard_access()
    {
        ...
        foreach (var platform in new[] { "Windows", "Linux", "macOS" })
        {
            ...
            Assert.Contains(":webdav:", command); Assert.Contains("--webdav-user " + session.Code, command); Assert.Contains("--read-only", command);
            Assert.Contains(expectedUrl, command); Assert.DoesNotContain("rclone config", command);
            ...
            Assert.Contains(platform == "macOS" ? " nfsmount :webdav:" : " mount :webdav:", command);
